...
arguments:
  - position: 1
    ...
    valueFrom: >-
      ...
      FILT="##fileformat=VCFv4.2\n##FILTER=<ID=HDP,Description=\"High depth, ceiling exceeded based on formula DP > avg(DP) + sqrt(avg(DP)) * 4\">"

      zcat $(inputs.input_vcf.path) | grep -E "^#" | sed -E "s@##fileformat.*@$FILT@" > header.txt
      ...
      perl -e 'open(AVG, "<", "avg.txt"); $avg = <AVG>; chomp $avg; $cutoff=($avg + (sqrt($avg) * 4)); print STDERR "Hi depth cutfoff is $cutoff\n"; open(VCF, "<", "body.vcf"); open(HI, ">>", "$(inputs.input_vcf.nameroot).hi_depth.vcf"); while(<VCF>){@line = split /\t/, $_; @info = split /;/, $line[7]; @dp_info = split /=/, $info[0]; if($dp_info[1] <= $cutoff){print $_;} else{$line[6] = "HDP";print HI join("\t", @line);}} close HI;' > depth_passed.vcf
